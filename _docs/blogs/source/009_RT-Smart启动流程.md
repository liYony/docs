...
在程序的入口地址0xc0000000处执行的指令是b \_reset，所以程序的启动是从\_reset函数开始的，下面对_reset函数进行简化并调整部分顺序(不影响功能)，代码如下：
...
在内核最开始初始化MMU的时候，我们只需要保证cpu发出的地址经过MMU能准确访问到内存即可，不需要什么内存管理，所以我们可以直接使用段映射来完成这个功能。
...
至于这里为什么要使用两段地址映射，后面在enable_mmu函数部分讲解。
...
里面就是初始化MMU的代码，主要就是配置各种寄存器，前面建立两段物理映射的目的也正是来保证mmu的成功初始化，详细解释如下：

首先因为我们的程序是烧写在物理内存的地址0x80001000上面的，而且在uboot里面使用了b 0x80001000的指令来从0x80001000地址开始运行，所以pc的值最开始应该是0x80001000，然后在这个地址上一条一条的执行，这里先画个示意图。

![image-20240316162526081](figures/image-20240316162526081.png)

当执行enable_mmu函数的时候，在没有使能MMU的时候，cpu访问的地址直接可以到物理内存，但是一旦初始化MMU呢？cpu访问的地址就需要通过MMU查找页表中的内存访问到物理内存了，上图虚线中cpu访问的pc的值为0x8003929c这个地址，访问MMU的时候会去查找这个地址映射的物理地址，如果这个地址没有进行映射，那么就无法访问到后面的指令了，也就是最重要的mov pc, lr指令无法执行，甚至可能触发地址访问异常。所以必须映射两段内存，通过调用mov pc, lr指令，使得我们的程序真正意义上的搬到虚拟地址上运行了。只要这条指令成功执行，第二段的映射就可以取消了。
...
后面那段跟c++相关的不做分析了。然后就跳转到rtthread的c入口函数rtthread_startup了。
...
arch_kuser_init()主要是在内核地址空间调用rt_hw_mmu_map_auto函数来建立地址0xffff0000的映射，并且将\_\_kuser_helper_start标号和\_\_kuser_helper_end标号之间的代码段映射到地址0xffff0000。
...
\_\_kuser_helper_start标号和\_\_kuser_helper_end标号之间定义了一些内核用户空间帮助器函数，这些函数在内核与用户空间之间交换数据时很有用。这里，我们看到一些用于原子操作、内存屏障和获取线程本地存储（TLS）的函数。
